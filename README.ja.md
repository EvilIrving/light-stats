...
通常のサンプリングには macOS ネイティブ API を使用し、サードパーティのランタイム依存はありません。モニタリングは読み取り専用のコアで、それ以外の機能（ウィンドウ管理、スクロール反転、出口ノード検出、AI 使用量）はすべて**既定でオフ**です。クリーンインストール直後はメニューバーの表示だけが動作し、余計なアイコンも、アクセシビリティの許可要求も、イベント tap も、（無効化できる）更新確認以外の外部リクエストもありません。
...
設定は「モニタリング（コア）」と「追加ツール（既定でオフ）」に分かれており、どの機能が任意かが一目で分かります。

...
일상적인 샘플링에는 macOS 네이티브 API를 사용하고 서드파티 런타임 의존성이 없습니다. 모니터링은 읽기 전용 코어이며, 그 외의 모든 기능(창 관리, 스크롤 반전, 출구 노드 감지, AI 사용량)은 **기본적으로 꺼져 있습니다**. 새로 설치한 직후에는 메뉴 막대 표시만 동작하며, 추가 아이콘도, 손쉬운 사용 권한 요청도, 이벤트 tap도, (끌 수 있는) 업데이트 확인 외의 외부 요청도 없습니다.
...
설정은 "모니터링(코어)"과 "추가 도구(기본 꺼짐)"로 나뉘어 어떤 기능이 선택 사항인지 한눈에 알 수 있습니다.

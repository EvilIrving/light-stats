...
- **Developer context in the menu bar.** Subscription windows and balances for 15 AI providers sit beside proxy and exit-node context; Finder actions open terminals, create files from type-aware and user templates (including Word / Excel / PowerPoint), and route selected files without leaving Finder.
- **Opt-in by construction.** Monitoring is read-only and always on; capabilities that touch the system or network are off until enabled and stop when disabled. Automatic update checks, Beta channel, AI provider requests, usage-window warmup, exit-node lookup, Finder actions, event taps, default input source, display brightness, and keep-awake assertions do not run on a clean install.
...
- **Trust through transparency.** Every claim about data and network is concrete and verifiable, including the one feature that does reach out. The diagnostic journal is always local, redacted, and user-exportable; the 48-hour performance recording is a separate opt-in session.

...
enum MemoryInfo {
    ...
    static func getMemoryInfo() -> Info {
        ...
        let used = MemoryUsagePolicy.usedBytes(
            total: total, anonymous: UInt64(stats.internal_page_count) * pageSize,
    ...
    static func getDetailedMemoryInfo() -> DetailedInfo {
        ...
        let used = MemoryUsagePolicy.usedBytes(
            total: total, anonymous: UInt64(stats.internal_page_count) * pageSize,
            purgeable: purgeable, wired: wired, compressed: compressed
        )
        let usagePercent = Double(used) / Double(total) * 100

...
# Checks that Localizable.strings keys are consistent across en, zh-Hans, ja.
...
if [ "$FAIL" -eq 0 ]; then
    ...
    echo "  ✅ All $EN_COUNT keys present in en, zh-Hans, ja"
else
    echo ""
    echo "Run: ./validate_localization.sh"
    echo "Fix: add missing keys to all three .lproj/Localizable.strings files"
    exit 1
fi

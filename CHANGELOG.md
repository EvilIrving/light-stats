...
- AI 用量扩到 15 家：编译期 provider 登记表，一家一开关，默认全关；API token 型在设置页粘贴、存 Keychain；Cursor 读本地 `state.vscdb`
- 概览用量行折叠：多窗口只显示最紧的一条（图标、名称、进度、百分比与重置倒计时同行），点击展开全部窗口；余额型沉底、只显示金额不画进度条
- 右键菜单：新建 Word / Excel / PowerPoint 与自定义文件模板（独立副本，保留原名、格式和内容）；移动/复制到常用目录或「其他位置…」；常用目录直达；隐藏/显示选中项
- 默认输入法：切换 App 后回到选定的系统输入源；默认关闭，只在激活后的短窗口内纠正漂移
- 清理快捷键：可选全局热键，在指针处打开清理页，不必点菜单栏；默认关闭，不需要辅助功能权限
- 窗口管理改为原生优先：先按 `AXIdentifier` 按下 macOS 窗口菜单分屏，失败再走自有几何；设置页露出系统拖到边缘分屏；标题栏手势从红绿灯现算高度，按住 Fn 吸附指针下的窗口
- 诊断报告：故障日志始终开启，设置中可导出 ZIP（系统环境、硬件探测、最近 7 天日志）；48 小时性能记录与故障日志永久分离

### 🔧 调整 / Changed

- Cursor 主窗口口径改为 auto（Cursor Models）百分比，API 用量另起一行（可为 0）
- 仅支持 Apple Silicon：`ARCHS = arm64`，不再产出 x86_64 切片；私有 API 显示器亮度控制不再保留 x86_64 空实现回退

### 🐛 修复 / Fixed

- 应用退出恢复为即时完成，避免应用内更新停在「正在校验并安装」
- Grok SuperGrok 统一计费在周额度刚重置、接口省略 0% 时不再显示为「—」
...
- 离线激活码（Ed25519 本地校验、零网络请求）与 Pro 赠送期：正式收费前启动过 App 的用户永久获赠 Pro
- 应用内更新改为读取 Cloudflare R2 渠道标记（`latest-stable.json` / `latest-beta.json`，含 SHA-256 校验），GitHub Releases 保留为回退

### 🔧 调整 / Changed

- 移除电池充电管理及其特权 helper；保留电量、循环次数、健康度、功率和温度等只读监控
- 健康分降低系统负载维度权重；总览负载读数改为每核归一化
- Codex 显示名称改为 ChatGPT

### 🐛 修复 / Fixed

- 修复虚拟显示器崩溃，并统一输入设备关闭路径
- 显示器 DDC 在总线挂起后停止 I/O
- 趋势历史可靠恢复；快捷键标签简化

...
诊断报告由用户主动导出，包含系统/硬件环境、现场探测、非敏感设置及最近 7 天诊断日志（总上限 50 MB），不得包含序列号、凭据、用户名或完整 home 路径。性能记录是另一套产品能力：仅在用户主动开始 48 小时记录时采集本软件 CPU、内存、唤醒、磁盘及配套系统负载，用独立 schema 和 `Performance Recordings` 目录保存 14 天；不得写回故障日志，也不得进入诊断报告。以后新增采集器时必须遵守这条边界。

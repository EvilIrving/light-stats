...
<img src="design/app-icon.png" width="120" alt="Light Stats" />

# Light Stats

**原生 macOS 菜单栏状态仪表,看的是「你的 Mac 此刻卡不卡」,并把开发工作流上下文放在手边。**

0-100 健康分常驻菜单栏,一眼看压力;点开弹窗查看 CPU、GPU、内存压力、磁盘与 I/O、网络、电池、温度、风扇、进程和 AI CLI 用量,还可按需启用 Finder、窗口、默认输入法、显示器亮度与保持唤醒工具。弹窗支持「经典」「黄金时刻」「琥珀」「墨夜」四种视觉主题,干净安装默认使用「墨夜」;设置窗保持系统白底工具面板。仅支持 Apple Silicon。
...
- 可选内建/外接屏硬件亮度(DDC,默认关闭)
- 温度、风扇、热状态和磁盘状态条
- 系统健康分、各维度摘要和维度开关
- 开启 AI 监控后显示 15 家 AI 订阅用量（额度窗口与预付余额）
- 关键指标短期趋势折线
...
- 产品表面(弹窗、关于、Toast、更新窗、辅助功能引导)可选四种主题:
  - **经典**:系统 instrument 读数(无卡片框；macOS 26+ 为 Liquid Glass，macOS 15 为普通系统表面)
  - **黄金时刻**:动态光场主题
  - **琥珀**:动态光场主题
...
- 正常关闭和二次确认后的强制退出
- 可展开查看子进程
- 列表采用 instrument 读数布局:分区、发丝线、密集行

### Finder 右键菜单

- 可选 FinderSync 扩展,默认关闭
- 复制选中文件的路径或名称,空白处可复制当前目录路径
- 新建文本、代码及 Word / Excel / PowerPoint 文件;默认仅显示 TXT / Markdown,其他类型可在设置中勾选
- 从现有文件添加自定义模板,保留原名、格式和内容;原文件移动或删除不影响模板
- 移动或复制到常用目录,也可通过「其他位置…」临时选择目标
- 直接打开常用目录,或用所选终端、配置的 App 打开当前位置和文件
- 隐藏/取消隐藏选中项;显示隐藏文件会经确认后更改访达全局设置并重新打开访达
- 可选 cmux 新窗口和新工作区操作;可分别关闭不用的菜单项目
- 覆盖主目录、外接卷及添加的常用目录;云盘目录仍受其他 Finder 扩展和系统状态影响
- 设置页显示扩展注册状态,并提供重新打开访达的入口

### 窗口管理

- 一个**「窗口管理」总开关**(默认关闭)同时启用三样:菜单栏窗口控制图标、全局贴靠快捷键和标题栏手势,没有单独的子开关
- 放置优先走 macOS 自己的窗口菜单分屏(按 `AXIdentifier` 匹配,不看本地化标题);只有三分之一、跨显示器移动和最小化才用应用自己的几何
- 菜单栏窗口控制入口,菜单动作带设计师绘制的图标
- 左半屏、右半屏、上半屏、下半屏保留快捷键,覆盖最高频窗口放置操作
- `Control + Option + Return` 最大化当前窗口,`Control + Option + C` 将当前窗口居中
- 菜单提供角落、三分之一、跨显示器移动、最大化、居中、还原、最小化等动作
- 标题栏触控板滑动:标题栏高度从窗口红绿灯现算,点在应用自己会响应的控件上不起手。按住 **Fn** 改为吸附指针下的窗口,用来覆盖自绘标题栏的 App
- 设置页同时露出 macOS 自带的拖到边缘分屏开关(macOS 15+);第一次打开窗口管理时会把边缘拖拽打开一次
- 关闭总开关会立即移除图标并停止所有窗口控制 event tap;辅助功能权限只在你打开开关时才请求

### 默认输入法

- 可选:切换 App 后回到选定的那一个系统输入源
- 默认关闭,不需要辅助功能权限
- 只在 App 激活后的短窗口内纠正漂移,之后的 `Ctrl+Space` 不受影响;安全输入框从不触碰
- 只做全局默认,不是输入法管理器:没有按 App、按网站的规则,也没有指示器

### 滚动方向控制

- 垂直和水平滚动方向可独立反转
- 可选择是否将触控板和 Magic Mouse 纳入方向反转
- 可关闭滚轮加速,并将每次滚动固定为 1-10 行
- 步长倍率用于微调滚动手感
- 仅在相关功能开启时启动事件 tap

### 找到我的鼠标

- 自行录制触发键(左/右修饰键、Fn、F 键、普通键或组合键):双按后全屏变暗并聚光指针,三按切换常驻演示指针
- 聚光灯跟随指针移动，点击或按任意键后淡出，另有 4 秒安全超时；演示指针再次三按即可关闭
- 不新增快捷键或独立设置项；仅监听事件 tap，绝不拦截或改写事件，需辅助功能权限且默认关闭
- Pro 功能：当前赠送期内启动过 App 的用户永久获赠 Pro；正式收费版本发布后，新用户需激活码解锁（设置 → 通用）。App 保持 MIT 开源；激活码用 Ed25519 签名离线校验，零网络请求

### 清理快捷键

- 可选全局热键，在指针处打开清理页，不必点菜单栏
- 默认关闭；Carbon 热键，不需要辅助功能权限
- 默认 ⌃⌥⌘U，可自行录制
...
- 弹窗工具栏提供「保持唤醒」快捷开关,可临时阻止显示器进入睡眠,无需辅助功能权限
- 关闭工具栏开关或退出 Light Stats 时立即停止
- 登录时启动可在设置中通过 macOS 原生登录项服务配置

### 更新

- 手动检查或选择开启自动检查,读取 Cloudflare R2 渠道标记(`latest-stable.json` / `latest-beta.json`),GitHub Releases 作为回退;自动检查默认关闭
- 更新通道:**正式版**(仅正式发布)或 **尝鲜 / Beta**(同时纳入 `v1.9.0-beta.N` 等预发布)
- SemVer 2.0 正确比较预发布标识,beta 递增与转正可识别
- R2 下载另校验 SHA-256,再验证 codesign 签名、公证状态和 Team ID
- 应用退出后通过独立脚本替换运行中的 app bundle
- 下载和安装期间显示轻量进度窗口

### 诊断与性能记录

- 本地结构化故障日志始终开启(没有关 / 仅错误 / 完整三级)。连续指标按时间采样;能力与探测在首次观察和状态变化时写入
- 设置中可导出诊断报告 ZIP(系统环境、硬件探测、最近 7 天日志,总上限 50 MB),序列号、凭据、用户名和完整 home 路径会脱敏,不会上传
- 另有可选的 48 小时性能记录,只记本软件 CPU、内存、唤醒、磁盘及配套系统负载;不写回故障日志,也不进入诊断报告
...
- **不是输入法,也不是输入法管理器。** 只在系统已有的输入源里选一个全局默认。
- **不支持 macOS 14 以前的系统,也不支持 Intel Mac。**
...
从 [download.onecat.dev/stable](https://download.onecat.dev/stable)(或 [GitHub Releases](https://github.com/EvilIrving/light-stats/releases/latest)) 下载最新 DMG,拖入「应用程序」即可。安装包经过签名与公证;内置自动更新会在校验 SHA-256(R2)、codesign、公证状态和 Team ID 之后才替换自身。

要求:macOS 14 或更新版本的 Apple Silicon Mac。
...
- 视觉主题(经典 / 黄金时刻 / 琥珀 / 墨夜),以及黄金时刻、琥珀与墨夜的颗粒、光影动态
- 菜单栏项目显示开关
- 刷新频率:低 (5s)、中 (2s)、高 (1s)
- 温度单位:摄氏度或华氏度
- 登录时启动、自动检查更新和正式版/尝鲜通道
- 导出诊断报告;可选 48 小时性能记录
- 出口节点探测和 provider 选择
- 15 家 AI 监控,以及 Claude/Codex 独立的用量窗口保活开关
- 垂直与水平滚动方向独立反转,可选纳入触控板和 Magic Mouse,关闭滚轮加速后固定 1-10 行,以及步长倍率
- 切换 App 后回到选定输入法
- 清理快捷键：在指针处打开清理页，不必点菜单栏
- 窗口管理(单一开关:菜单栏图标、贴靠快捷键、标题栏手势),以及系统自带的边缘分屏开关
- 显示器硬件亮度(DDC)
- Finder 菜单、终端选择、cmux 操作、常用目录、App 和文件模板
- 健康分维度开关
- 语言:简体中文、English、日本語、한국어、跟随系统

设置侧栏提供六个直达入口:「通用」「监控」「输入设备」「窗口管理」「AI 用量」「右键菜单」,不显示状态点。窗口固定系统白底与浅色侧栏;只有弹窗等产品表面跟随所选主题。「保持唤醒」位于弹窗工具栏,不是设置项。

...
**原生 macOS 菜单栏系统监控,看的是「你的 Mac 此刻卡不卡」,不只是「用了多少」。**

0-100 健康分常驻菜单栏,一眼看压力;点开弹窗看全貌:CPU、GPU、内存压力、磁盘与 I/O、网络、代理与出口节点、电池、温度、风扇、进程、AI CLI 用量。
...
- **默认不联网。** 没有遥测;出口节点探测和 AI 用量监控默认都是关的;只有检查更新会访问 GitHub Releases。
- **监控是核心,附加工具默认全关。** 除了只读监控,它还顺带提供清洁模式、窗口管理(图标 + 快捷键 + 标题栏手势)和滚动方向反转——但**这些默认全部关闭**。只用监控的话,菜单栏只有读数:不多一个图标、不弹辅助功能授权、不创建任何 event tap、除可关的更新检查外不发任何请求。
...
- **出口节点探测默认关闭。** 开启后,应用会向所选 geo-IP provider 发送请求,用于识别当前公网 IP 和网络归属。结果缓存 60 秒,请求失败时静默降级。
- **AI 用量监控默认关闭。** 开启后,请求只会发送到对应 provider 自己的用量接口,使用该 provider CLI 已经保存在本机的凭据,不会传输到其他服务。
- **更新检查** 会访问 GitHub Releases。
...
设置页分为「监控(核心)」与「附加工具(默认关闭)」两组,一眼看出哪些能力是可选的。
